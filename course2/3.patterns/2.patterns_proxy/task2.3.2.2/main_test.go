...
func TestGithubAdapter_GetGists(t *testing.T) {
	...
	tests := []struct {
		...
		{"Test github adapter mock get gists", GithubProxy{GithubAdapter{GistList: &MockGistLister{false}}, make(map[string][]Item)}, args{context.Background(), "username"}, []Item{
		...
		{"Test github adapter mock get gists error", GithubProxy{GithubAdapter{GistList: &MockGistLister{true}}, make(map[string][]Item)}, args{context.Background(), "username"}, nil, true},
	}
...
func TestGithubAdapter_GetRepos(t *testing.T) {
	...
	tests := []struct {
		...
		{"Test github adapter mock get repos", GithubProxy{GithubAdapter{RepoList: &MockRepoLister{false}}, make(map[string][]Item)}, args{context.Background(), "username"}, []Item{
		...
		{"Test github adapter mock get repos error", GithubProxy{GithubAdapter{RepoList: &MockRepoLister{true}}, make(map[string][]Item)}, args{context.Background(), "username"}, nil, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
