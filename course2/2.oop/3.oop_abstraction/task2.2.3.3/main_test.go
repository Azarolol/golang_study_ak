...
func TestSQLiteGenerator_CreateTableSQL(t *testing.T) {
	...
	tests := []struct {
		...
		{"Test SQLite generator create table", SQLiteGenerator{}, args{&User{}}, "CREATE TABLE IF NOT EXISTS users (id SERIAL PRIMARY KEY, first_name VARCHAR(100), last_name VARCHAR(100), email VARCHAR(100) UNIQUE);"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
