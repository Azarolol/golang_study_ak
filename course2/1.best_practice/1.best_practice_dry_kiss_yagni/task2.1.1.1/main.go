...
func WithAverageProfit(s *StatisticProfit) {
	s.getAverageProfit = func() float64 { return s.Average(s.product.Sells) - s.Average(s.product.Buys) }
}
...
func WithAverageProfitPercent(s *StatisticProfit) {
	s.getAverageProfitPercent = func() float64 { return s.GetAverageProfit() / s.Average(s.product.Buys) * 100 }
}

// WithCurrentProfit sets the current profit of the product
//
// Current Profit = Currnet Price - Current Price * (100 - Profit Percent) / 100
func WithCurrentProfit(s *StatisticProfit) {
	s.getCurrentProfit = func() float64 {
		return s.product.CurrentPrice - s.product.CurrentPrice*(100-s.GetAverageProfitPercent())/100
	}
}
...
func WithDifferenceProfit(s *StatisticProfit) {
	s.getDifferenceProfit = func() float64 { return s.product.CurrentPrice - s.Average(s.product.Sells) }
}
